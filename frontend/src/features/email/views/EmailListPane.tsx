...
const EmailListPane = () => {
  ...
  return (
    <div class="h-full flex flex-col bg-surface-0 dark:bg-surface-950">
      ...
      {/* Scroll surface: isolated paint + compositor hint for smooth virtualized scrolling */}
      <div
          ...
          fallback={
            ...
            >
              <Index each={virtualizer.getVirtualItems()}>
                {(virtualRow) => {
                  const isSentinel = createMemo(() => virtualRow().index === emails().length);
                  const email = createMemo(() => emails()[virtualRow().index]);
                  return (
                    <Show
                      when={!isSentinel()}
                      fallback={
                        <div
                          ...
                          style={{
                            ...
                            /* Kill the global * { transition: transform } rule so
                               row repositioning is instant, not a 150ms animation. */
                            transition: "none",
                          }}
                          classList={{
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          width: "100%",
                          transform: `translateY(${virtualRow().start}px)`,
                          /* Instant reposition — see note above. */
                          transition: "none",
                        }}
                      >
                        <Show when={email()}>
                          <EmailListItem
                            ...
                            checked={
                              email()!.id
                                ? selectedIds().has(email()!.id!.toString())
                                : false
                            }
                          />
                        </Show>
