...
function updateCursor() {
    ...
    // Only disable text selection and native touch actions if we are actually able to pan
...
document.addEventListener('pointerdown', function(e) {
    ...
    // Allow interacting with form elements even in pan mode
    ...
    // Robustly capture the current scroll position across different browser engines
    ...
    // CRITICAL: Prevent default immediately to stop native image dragging, link dragging,
    // and text selection. This guarantees smooth panning. We will manually simulate a
    // click event on pointerup if the user didn't drag.
    e.preventDefault();
...
document.addEventListener('pointermove', function(e) {
    ...
    if (hasDragged) {
        ...
        // Force scroll across all possible viewport containers to guarantee it works in WebKitGTK
...
function endPan(e) {
    ...
    // If the user just clicked without dragging, manually dispatch a click event.
    // This restores link interactivity that was suppressed by preventDefault() on pointerdown.
    if (!hasDragged) {
